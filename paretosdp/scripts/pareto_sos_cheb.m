...
function [pout_discret, p1lam, p2lam, f1val, f2val, pxlam, mx] = pareto_sos_cheb (x, xfree, lambda, s, d, N, n, f1, f2, f1free, f2free, Khat, lam_begin, lam_end, lo1, up1, lo2, up2, inverse_method)
C = max (up1 - lo1, up2 - lo2);
%pars.eps = 1e-6; mset(pars);
mpol('omega', 1);
x = [x; omega]; % x is an n+1 polynomial vector
mu = meas(x, lambda);
mufree = meas(xfree);
...
cheb_g1 = C * x(n+1) - lambda * (f1 - lo1) >= 0; cheb_g2 = C * x(n+1) - (1 - lambda) * (f2 - lo2) >= 0;
cheb_b1 = x(n+1) >= 0; cheb_b2 = 1 - x(n+1) >= 0;
Khat = [Khat, cheb_g1, cheb_g2, cheb_b1, cheb_b2, x'*x + lambda^2 <= n + 2];
[P, M] = msdp(min(flambda), Khat, mmom_cstr', d);
[A,b,c,K] = msedumi(P);
